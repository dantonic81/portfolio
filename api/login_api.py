...
def send_confirmation_email(username, email, token):
    confirm_link = url_for(
        "login_api.confirm_email", token=token, email=email, _external=True
    )
...
def confirm_email():
    ...
    try:
        ...
        cursor.execute(
            "UPDATE users SET is_active = 1, confirmation_token = NULL, token_expiry = NULL WHERE confirmation_token = ?",
            (token,),
        )
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if existing_user:
                ...
                if not token_expiry_str or datetime.strptime(
                    token_expiry_str, "%Y-%m-%d %H:%M:%S.%f%z"
                    ...
                    new_expiration_time = datetime.now(timezone.utc) + timedelta(
                        minutes=5
                    )  # New token expires in 5 minutes
                    ...
                    confirm_link = url_for(
                        "login_api.confirm_email", token=new_token, _external=True
                    )
                    email_content = render_template(
                        "email.html", username=username, confirm_link=confirm_link
                    )
                    ...
                    flash(
                        "Confirmation link has not expired. Please use the existing link.",
                        "error",
                    )
                    return render_template(
                        "register.html", error="Confirmation link has not expired."
                    )
            else:
                ...
                # Render the HTML email template
                confirm_link = url_for(
                    "login_api.confirm_email", token=token, _external=True
                )
                email_content = render_template(
                    "email.html", username=username, confirm_link=confirm_link
                )
            ...
            log_audit_event(
                request, "registration_attempt", username, "failure", str(e)
            )
        ...
        try:
            ...
            if user and check_password_hash(user[1], password):
                session["user_id"] = user[
                    2
                ]  # Store user_id in session, not just username
                session["username"] = user[0]
                log_audit_event(request, "login_attempt", username, "success")
                flash("Login successful!", "success")
                return redirect(
                    url_for("api.index")
                )  # Replace 'api.index' with your desired endpoint
            else:
                log_audit_event(
                    request, "login_attempt", username, "failure", "Invalid credentials"
                )
